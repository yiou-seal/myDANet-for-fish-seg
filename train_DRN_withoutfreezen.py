...
if __name__ == "__main__":
    ...
    savemodepath='imagenetpretrained_shujzengyi_excludenofish_lr5e_5_adam_try2_logs/'

    # model   = DeepLab(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor, pretrained=pretrained)
    model = DANetwp(nclass=num_classes, backbone='resnet50',baseisperTrain=pretrained)
    # model = DANet(nclass=num_classes,baseispertrained=pretrained)
    print(model)
    if not pretrained:
        weights_init(model)
    if pretrained:
