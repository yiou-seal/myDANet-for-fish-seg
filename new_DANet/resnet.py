...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        # 这里判断一下是不是有通道数变化，如果有就把残差连接那条路径上添加上一个1x1卷积调整一下通道。
        downsample = None
    ...
    def _make_deeplabv3_layer(self, block, planes, blocks, rates, stride=1):
        ...
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, rate=rates[i])) # rates：空洞卷积的空洞率
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load("./pertrained_model/resnet50-19c8e357.pth"))
    return model
