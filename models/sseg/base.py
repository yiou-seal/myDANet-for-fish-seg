...
# 模型主体部分的的基本骨架，不包括attention
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
                 base_size=520, crop_size=480, mean=[.485, .456, .406],
                 std=[.229, .224, .225], root='~/.encoding/models', basenetispretrain=False, *args, **kwargs):
        ...
        # 根据backbone指定的骨干网络名，把对应的骨干网络对象取出来
        self.pretrained = get_backbone(backbone, pretrained=basenetispretrain, dilated=dilated,
    ...
    def base_forward(self, x):
        ...
        else: # ResNet用这个，注意到这里已经把原始ResNet的avgpool(x)与flatten(x, 1)，Linear层舍弃了
                ...
                for idh in range(h_grids):
                    for idw in range(w_grids):
                        ...
                        pad_crop_img = pad_image(crop_img, self.module.mean,
                                                 self.module.std, crop_size)
                        output = module_inference(self.module, pad_crop_img, self.flip)# 走前向传播
...
def module_inference(module, image, flip=True):
    ...
    if flip:
        fimg = flip_image(image) # flip_image翻转图像
