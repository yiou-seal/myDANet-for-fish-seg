...
class FCNs(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.bn1(self.relu(self.deconv1(x5)))     # size=(N, 512, x.H/16, x.W/16)
        score=interpolate(score, x4.size()[2:], mode='bicubic', align_corners=True)
        score = score + x4                                # element-wise add, size=(N, 512, x.H/16, x.W/16)
        score = self.bn2(self.relu(self.deconv2(score)))  # size=(N, 256, x.H/8, x.W/8)
        # ups = torch.nn.Upsample(size=x3.size(),mode='bicubic')
        # score = ups(score)
        score = interpolate(score, x3.size()[2:], mode='bicubic', align_corners=True)
        score = score + x3                                # element-wise add, size=(N, 256, x.H/8, x.W/8)
        score = self.bn3(self.relu(self.deconv3(score)))  # size=(N, 128, x.H/4, x.W/4)
        # ups = torch.nn.Upsample(size=x2.size(), mode='bicubic')
        # score = ups(score)
        score = interpolate(score, x2.size()[2:], mode='bicubic', align_corners=True)
        score = score + x2                                # element-wise add, size=(N, 128, x.H/4, x.W/4)
        score = self.bn4(self.relu(self.deconv4(score)))  # size=(N, 64, x.H/2, x.W/2)
        # ups = torch.nn.Upsample(size=x1.size(), mode='bicubic')
        # score = ups(score)
        score = interpolate(score, x1.size()[2:], mode='bicubic', align_corners=True)
